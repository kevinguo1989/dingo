...
public abstract class TableCoordinatorServerApi {
        ...
        if (state == 1) {
            synchronized (this) {
                ...
                for (String columnName : columnNames) {
                    if (!tableDefinition.getColumnDefinitionMap().containsKey(columnName)) {
    ...
    public void deleteIndex(String tableName, String indexName) {
        if (state == 1) {
            synchronized (this) {
                TableDefinition definition = getTableDefinition(tableName);
                definition.setIndexDelete(indexName);
    ...
    void reboot(String tableName) throws Exception {
        ...
        while (getTableDefinition(tableName).getNonDeleteIndexCount() != 0) {
        ...
        TableDefinition definition = getTableDefinition(tableName);
        KeyValueCodec codec = new DingoKeyValueCodec(definition.getDingoType(), definition.getKeyMapping());
        for (String indexName : definition.getIndexMap().keySet()) {
            KeyValueCodec indexCodec = null;
        ...
        List<String> busyIndexNames = definition.getBusyIndex();
        ...
        // 9. 删除标记未delete的index
        List<String> deleteIndexNames = definition.getDeleteIndex();
    ...
    public abstract Map<String, String> getChangedIndexExecutorAddr(String tableName, List<String> changedIndexName);
}
