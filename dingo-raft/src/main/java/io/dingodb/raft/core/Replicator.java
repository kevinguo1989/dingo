...
import io.dingodb.raft.util.RecyclableByteBufferList;
...
public class Replicator implements ThreadId.OnError {
     ...
     */
    private static void notifyReplicatorStatusListener(final Replicator replicator, final ReplicatorEvent event,
                                                       final Status status, final ReplicatorStateListener.ReplicatorState newState) {
    ...
    void setState(final State state) {
        ...
        if(oldState != state) {
          ReplicatorStateListener.ReplicatorState newState = null;
          switch (state) {
            case Created:
              newState = ReplicatorStateListener.ReplicatorState.CREATED;
              break;
            case Replicate:
            case Snapshot:
              newState = ReplicatorStateListener.ReplicatorState.ONLINE;
              break;
            case Probe:
              newState = ReplicatorStateListener.ReplicatorState.OFFLINE;
              break;
            case Destroyed:
              newState = ReplicatorStateListener.ReplicatorState.DESTROYED;
              break;
          }
    ...
    private void addInflight(final RequestType reqType, final long startIndex, final int count, final int size,
        ...
        this.nodeMetrics.recordSize(MetricRegistry.name(this.metricName, "replicate-inflights-count"), this.inflights.size());
    }
    ...
    private boolean sendEntries(final long nextSendingIndex) {
        LOG.info("Send Entries to : " + this.options.getPeerId().getEndpoint() + " with Index : " + nextSendingIndex);
        final RpcRequests.AppendEntriesRequest.Builder rb = RpcRequests.AppendEntriesRequest.newBuilder();
    ...
    private Future<Message> timeoutNow(final RpcRequests.TimeoutNowRequest.Builder rb, final boolean stopAfterFinish,
                                       final int timeoutMs) {
        ...
        return this.rpcService.timeoutNow(this.options.getPeerId().getEndpoint(), request, timeoutMs,
            new RpcResponseClosureAdapter<RpcRequests.TimeoutNowResponse>() {
    ...
    @SuppressWarnings("unused")
    static void onTimeoutNowReturned(final ThreadId id, final Status status, final RpcRequests.TimeoutNowRequest request,
                                     final RpcRequests.TimeoutNowResponse response, final boolean stopAfterFinish) {
        final Replicator r = (Replicator) id.lock();
        if (r == null) {
            return;
        }
